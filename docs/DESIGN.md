...
- **Hero band** (`--background`): a single centered column (name, bio, contact icons), capped at 65ch. A two-column hero with a graphic anchor (an arch-shaped monogram standing in for a photo) was tried and dropped — it read as an odd "tombstone" shape rather than a visual anchor. Centered and text-only reads cleaner.
...
- **Don't** add a real photograph, stone/marble texture, a curved SVG section divider, emoji, a hand-drawn signature graphic, a graphic "hero mark" replacing a photo, or a filled/outlined chip around a section heading — all tried or considered and dropped. Section Heading color/underline variation is the one piece of layout personality this system uses; don't add a decorative device on top of it without cutting one first.
