...
### Contact Icons
- **Shape:** 36×36px hit target (`Button` `size="icon-lg"`), `rounded-lg` corners — the shared button radius, not a circle. Rendered as an `<a>` via the Button's `render` prop with `nativeButton={false}`, so the link keeps anchor semantics.
- **Style:** icon inherits page `--foreground` at rest — no fill, no labels, no pill chrome. On hover the `ghost` variant fills `--muted` (`--muted/50` in dark) and text stays `--foreground`; focus-visible draws a `--ring` ring. The accent does *not* appear here — unlike the row arrow, these icons don't turn primary on hover.
